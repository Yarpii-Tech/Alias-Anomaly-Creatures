...
if(SC_occupyTraders) then{call compile preprocessFileLineNumbers "\x\addons\a3_exile_occupation\scripts\occupationTraders.sqf";};
...
[_logDetail] call SC_fnc_log;
if(SC_occupyLootCratesMarkers) then{uiSleep 15;
...
if(SC_occupyMilitary) then{uiSleep 15;
_logDetail = format ["[OCCUPATION Military]:: Starting building scan @ ",time];
[_logDetail] call SC_fnc_log;
_middle     = worldSize/2;
_areaToScan     = [_middle,_middle,0];
_maxDistance     = 20000;
...
if(SC_occupyRandomSpawn) then{uiSleep 15;
...
if(SC_occupyStatic) then{uiSleep 15;
...
if(SC_occupySky) then{uiSleep 15;
...
if(SC_occupySea) then{uiSleep 15;
...
if(SC_occupyPlaces) then{uiSleep 15;
...
if(SC_occupyTransport) then{uiSleep 15;
